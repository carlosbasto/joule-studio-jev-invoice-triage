...
Behaviour is controlled by the IBD_TESTING environment variable:
  ...
  Real integration (IBD_TESTING=0):
  ...
  Public demo / test mode (IBD_TESTING unset or truthy):
...
def _mock_mode() -> bool:
    """Public sample defaults to bundled mock tools unless explicitly disabled."""
    return os.environ.get("IBD_TESTING", "1").strip().lower() in {"1", "true", "yes", "on"}
